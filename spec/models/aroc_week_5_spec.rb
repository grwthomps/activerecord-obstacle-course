...
# ----------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    ordered_items_names = Item
      .joins(:order_items)
      .where(order_items: {item_id: Item.pluck(:id)})
      .distinct
      .order(:name)
      .pluck(:name)
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User
      .select('users.name AS user_name, orders.id AS order_id, orders.amount / 4 AS avg_item_cost')
      .joins(:orders, :order_items)
  ...
  it '30. returns the names of items that have been ordered without n+1 queries' do
    ...
    # ------------------------------------------------------
    orders = Order.all.includes(:order_items).includes(:items) # Edit only this line
    # ------------------------------------------------------
